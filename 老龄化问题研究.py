...
class Solution:
    def solve(self):
        csv_file1 = urllib.urlopen("http://py.mooctest.net:8081/dataset/population/population_old.csv")
        csv_file2 = urllib.urlopen("http://py.mooctest.net:8081/dataset/population/population_total.csv")
        i = 0
        raw_data_1 = []
        for line in csv_file1:
            ...
            if i <= 3:
                continue
            raw_data_1.append(line.split(',')[1])
        i = 0
        raw_data_2 = []
        for line in csv_file2:
            ...
            if i <= 5:
                continue
            raw_data_2.append(line.split(',')[4])
        raw_data = []
        for x in range(raw_data_1.__len__()):
            raw_data.append([int(raw_data_1[x]), int(raw_data_2[x])])
    ...
    def cal_old(self, data):
        return float(data[0]) * 100 / float(data[1])
